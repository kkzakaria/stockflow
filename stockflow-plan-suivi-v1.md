...
|---------|--------|-------------|--------|
| S1 (5-11 fév) | Fondations | ___/30 | ⬜ |
| S2 (12-18 fév) | Core Métier | ___/32 | ⬜ |
| S3 (19-25 fév) | Transferts, Inventaire & Résilience | ___/36 | ⬜ |
| S4 (26 fév - 5 mars) | Dashboard, Polish & Déploiement | ___/38 | ⬜ |
| **Total V1** | | **___/136** | ⬜ |
...
|---|-------|--------|----------|
| 1.1.1 | Créer le projet SvelteKit via `pnpm create cloudflare@latest stockflow -- --framework=svelte` | ⬜ | - [ ] |
| 1.1.2 | Installer les dépendances (drizzle-orm, better-auth, zod, dayjs, nanoid, html5-qrcode, idb, tailwindcss) | ⬜ | - [ ] |
| 1.1.3 | Créer la base D1 via `wrangler d1 create stockflow-db` | ⬜ | - [ ] |
| 1.1.4 | Configurer `wrangler.toml` (binding DB, nodejs_compat, vars) | ⬜ | - [ ] |
| 1.1.5 | Configurer `drizzle.config.ts` (schema path, dialect sqlite, dbCredentials) | ⬜ | - [ ] |
| 1.1.6 | Configurer les scripts npm (`db:push`, `db:generate`, `db:migrate:local`, `db:migrate:prod`, `db:studio`, `db:seed`) | ⬜ | - [ ] |
| 1.1.7 | Écrire le schéma Drizzle complet `src/lib/server/db/schema.ts` (source de vérité) | ⬜ | - [ ] |
| 1.1.8 | `db:push` + vérification via `db:studio` | ⬜ | - [ ] |
| 1.1.9 | Configurer Tailwind CSS + thème de base (`app.css`) | ⬜ | - [ ] |
| 1.1.10 | Configurer la structure de dossiers du projet (§7 du dev plan) | ⬜ | - [ ] |
| 1.1.11 | Initialiser Git + structure branches (main, develop, feat/*) | ⬜ | - [ ] |
...
- [ ] `pnpm dev` lance le serveur local (via `wrangler dev`) sans erreur
- [ ] Le schéma Drizzle complet est dans `src/lib/server/db/schema.ts`
- [ ] `pnpm db:push` synchronise la DB locale sans erreur
- [ ] `pnpm db:studio` affiche toutes les tables
- [ ] Tailwind fonctionne (classe utilitaire visible)
- [ ] Structure de dossiers conforme à la §7 du dev plan
- [ ] IDs en TEXT (nanoid), timestamps TEXT ISO, soft delete `is_active`
- [ ] Prix stockés en nombres (XOF, pas de centimes fractionnaires)
- [ ] Convention commits en place (feat/fix/chore/docs)
...
| # | Tâche | Statut | Checkbox |
|---|-------|--------|----------|
| 1.2.1 | Configurer Better Auth (`src/auth.ts`) avec drizzleAdapter, emailAndPassword, plugin admin | ⬜ | - [ ] |
| 1.2.2 | Créer la route catch-all `api/auth/[...betterauth]/+server.ts` | ⬜ | - [ ] |
| 1.2.3 | Implémenter `hooks.server.ts` — middleware session global | ⬜ | - [ ] |
| 1.2.4 | Créer la page Login (`/login`) — formulaire email/password | ⬜ | - [ ] |
...
- [ ] Connexion email/password fonctionnelle (Better Auth + sessions httpOnly + scrypt)
- [ ] Session validée sur chaque requête via `hooks.server.ts`
- [ ] Redirection `/login` si non authentifié sur les routes `(app)/`
- [ ] Réinitialisation de mot de passe complète (email → lien temporaire → reset)
- [ ] Déconnexion fonctionne (session terminée, redirection login)
- [ ] Sessions expirent après 7 jours, refresh quotidien
- [ ] Configuration `BETTER_AUTH_URL` et `BETTER_AUTH_SECRET` en place
...
| # | Tâche | Statut | Checkbox |
|---|-------|--------|----------|
| 1.3.1 | Créer le middleware RBAC (`src/lib/server/auth/rbac.ts`) | ⬜ | - [ ] |
| 1.3.2 | Implémenter les helpers : `requireRole()`, `requireWarehouseAccess()`, `canWrite()`, `canManage()`, `canApprove()`, `hasGlobalScope()` | ⬜ | - [ ] |
| 1.3.3 | Définir la hiérarchie des rôles (admin:100 → viewer:10) | ⬜ | - [ ] |
| 1.3.4 | Créer le layout principal desktop (`+layout.svelte`) — sidebar + header + slot | ⬜ | - [ ] |
| 1.3.5 | Créer le layout mobile — bottom navigation (5 onglets : Home, Stock, Trans, Inv, More) | ⬜ | - [ ] |
...
|------|------|--------|-------|------------|-------|
| Administrateur | `admin` | 100 | Global | - [ ] | - [ ] |
| Admin gestionnaire | `admin_manager` | 80 | Global | - [ ] | - [ ] |
| Gestionnaire | `manager` | 60 | Entrepôts assignés | - [ ] | - [ ] |
| Utilisateur | `user` | 40 | Entrepôt assigné | - [ ] | - [ ] |
| Admin visiteur | `admin_viewer` | 20 | Global (lecture) | - [ ] | - [ ] |
| Visiteur | `viewer` | 10 | Entrepôt assigné (lecture) | - [ ] | - [ ] |

### Critères d'acceptation

- [ ] `requireRole(userRole, minRole)` bloque avec 403 si rôle insuffisant
- [ ] `requireWarehouseAccess()` vérifie l'accès via table `user_warehouses` (bypass pour rôles globaux)
- [ ] `canWrite()` retourne false pour `admin_viewer` et `viewer`
- [ ] `canApprove()` retourne true uniquement pour `admin` et `admin_manager`
- [ ] Layout desktop : sidebar avec menu adapté au rôle (Users visible uniquement pour admin)
- [ ] Layout mobile : bottom nav 5 onglets, navigation au pouce
...
| # | Tâche | Statut | Checkbox |
|---|-------|--------|----------|
| 1.4.1 | API : `GET /api/v1/users` — liste paginée avec filtres (rôle, statut, entrepôt) | ⬜ | - [ ] |
| 1.4.2 | API : `POST /api/v1/users` — création + envoi email invitation | ⬜ | - [ ] |
| 1.4.3 | API : `GET /api/v1/users/[id]` — fiche complète | ⬜ | - [ ] |
| 1.4.4 | API : `PUT /api/v1/users/[id]` — modification (rôle, entrepôts, statut) | ⬜ | - [ ] |
| 1.4.5 | API : `DELETE /api/v1/users/[id]` — soft delete (is_active = 0) | ⬜ | - [ ] |
| 1.4.6 | API : `PUT /api/v1/users/[id]/warehouses` — assignation multi-entrepôts | ⬜ | - [ ] |
...
- [ ] Création déclenche un email d'invitation (Better Auth)
- [ ] Email unique validé côté serveur (Zod)
- [ ] Attribution de l'un des 6 rôles via liste déroulante
- [ ] Assignation multi-entrepôts via multi-sélection
- [ ] Désactivation empêche la connexion (is_active = 0)
- [ ] Liste paginée et filtrable (rôle, statut, entrepôt)
...
| # | Tâche | Statut | Checkbox |
|---|-------|--------|----------|
| 1.5.1 | API : `GET /api/v1/warehouses` — liste filtrée par permissions | ⬜ | - [ ] |
| 1.5.2 | API : `POST /api/v1/warehouses` — création (admin only) | ⬜ | - [ ] |
| 1.5.3 | API : `GET /api/v1/warehouses/[id]` — détail + résumé stock | ⬜ | - [ ] |
| 1.5.4 | API : `PUT /api/v1/warehouses/[id]` — modification (admin only) | ⬜ | - [ ] |
| 1.5.5 | API : `DELETE /api/v1/warehouses/[id]` — soft delete | ⬜ | - [ ] |
...
- [ ] Seul un admin peut créer/modifier/supprimer un entrepôt
- [ ] Un entrepôt a : nom, adresse, contact, statut (is_active)
- [ ] La liste est filtrée selon les permissions de l'utilisateur connecté
- [ ] Les rôles globaux voient tous les entrepôts ; les autres uniquement les entrepôts assignés
- [ ] Un entrepôt avec du stock ne peut pas être supprimé (erreur explicite)
...
|---------|----------|--------|
| `pnpm test` — tous les tests passent | ___ | - [ ] |
| `pnpm test:e2e` — tous les tests E2E passent | ___ | - [ ] |
| `pnpm build` — build réussi | ___ | - [ ] |
| `pnpm check` — svelte-check sans erreur | ___ | - [ ] |
| Migrations SQL vérifiées manuellement | ___ | - [ ] |
| Variables d'env configurées en prod | ___ | - [ ] |
...
|------|----------|--------|
| Login fonctionnel | ___ | - [ ] |
| Création produit | ___ | - [ ] |
| Mouvement entrée/sortie | ___ | - [ ] |
| Transfert complet (demande → réception) | ___ | - [ ] |
| Emails (reset password, alertes) | ___ | - [ ] |
| Scan codes-barres sur mobile réel | ___ | - [ ] |
| Test sur Android | ___ | - [ ] |
| Test sur iOS | ___ | - [ ] |
| Monitoring erreurs (Cloudflare Analytics) | ___ | - [ ] |
...
|---------|----------|----------|--------|
| Temps de réponse API | < 300ms | ___ms | ⬜ |
| Score Lighthouse Performance | > 80 | ___ | ⬜ |
| Score Lighthouse Accessibilité | > 80 | ___ | ⬜ |
| Uptime (24h test) | > 99% | ___% | ⬜ |
| Failles sécurité critiques | 0 | ___ | ⬜ |
| Taux scan réussi | > 90% | ___% | ⬜ |
| Perte données offline | 0 | ___ | ⬜ |
| Tests permissions (6 rôles × routes) | 100% | ___% | ⬜ |
...
| ⬜ | **NO-GO** — Reporter | Motif : ___ |

---

*Plan de suivi généré à partir du PRD StockFlow v1.0 et du Plan de Développement v1.0*  
*Dernière mise à jour : ___/___/2026*
