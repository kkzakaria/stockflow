...
export async function requireWarehouseAccess(userId: string, warehouseId: string, role: Role) {
	...
	const access = await db.query.userWarehouses.findFirst({
		where: (uw, { and, eq }) => and(eq(uw.userId, userId), eq(uw.warehouseId, warehouseId))
	});
...
<div class="scanner-container">
	...
	{#if !scanning}
		<button on:click={startScan} class="btn-scan"> 📷 Scanner un code-barres </button>
	{:else}
		<button on:click={stopScan} class="btn-stop"> ⏹ Arrêter le scan </button>
	{/if}
</div>
```
...
export const offlineQueue = {
	async enqueue(operation: { url: string; method: string; body: unknown; timestamp: string }) {
...
```
Semaine 1 ██████████████████████ 100% ✅ (PR #1 merged → main)
Semaine 2 ██████████████████████ 100% ✅ (PR #2 merged → main)
Semaine 3 ██████████████████████ 100% ✅ (PR #3 merged → main)
Semaine 4 ░░░░░░░░░░░░░░░░░░░░░   0% ⏳ (pas commencée)
Seed data ██████████████████████ 100% ✅ (PR #4 merged → main)
```

### 12.2 Checkpoints

| Date | Checkpoint | Statut | Notes |
|------|-----------|--------|-------|
| **7 fév** | CP0 — Setup validé | ✅ Atteint | Projet tourne, auth OK, db:push OK |
| **11 fév** | CP1 — Fondations | ✅ Atteint | CRUD users/entrepôts, layout responsive, 17 tâches |
| **16 fév** | CP2 — Core métier | ✅ Atteint | Produits, mouvements, stock service, PUMP, scan — 16 tâches, 56 tests |
| **17 fév** | CP3 — Workflows | ✅ Atteint | Transferts 8 statuts, inventaire, alertes, résilience offline — 14 tâches |
| **25 fév** | CP4 — Release Candidate | ⏳ À venir | Dashboard, logs UI, polish mobile, tests E2E |
| **5 mars** | CP5 — Go Live | ⏳ À venir | Déploiement production |

### 12.3 Semaine 1 — Fondations (5-11 fév) ✅

**Branche :** `feat/week1-foundations` → merged via PR #1 (16 commits)

| # | Tâche | Statut |
|---|-------|--------|
| 1 | Installer dépendances (better-auth, zod, nanoid, dayjs) | ✅ |
| 2 | Schéma Drizzle complet (16 tables, indexes, relations) | ✅ |
| 3 | Configurer Better Auth + Drizzle adapter | ✅ |
| 4 | Route catch-all Better Auth API | ✅ |
| 5 | Middleware session dans hooks.server.ts | ✅ |
| 6 | RBAC helpers (6 rôles, hiérarchie, scope entrepôt) + tests | ✅ |
| 7 | Auth guards (requireAuth, requireWarehouseAccess) | ✅ |
| 8 | Validators Zod (warehouse, user) | ✅ |
| 9 | API warehouse CRUD (GET list, POST, GET/:id, PUT/:id, DELETE/:id) | ✅ |
| 10 | API user management (CRUD + assignation entrepôts) | ✅ |
| 11 | Page login + layout auth | ✅ |
| 12 | Layout app (sidebar, header, bottom nav, protection routes) | ✅ |
| 13 | Dashboard page + stub pages + redirect root | ✅ |
| 14 | Composants UI de base (Button, Input, Modal, Badge, etc.) | ✅ |
| 15 | Pages entrepôts (liste, création, détail) | ✅ |
| 16 | Pages utilisateurs (liste, création, détail) | ✅ |
| 17 | Toast notifications | ✅ |

### 12.4 Semaine 2 — Core Business (12-18 fév) ✅

**Branche :** `feat/week2-core-business` → merged via PR #2 (17 commits)

| # | Tâche | Statut |
|---|-------|--------|
| 1 | Validators catégories + API catégories | ✅ |
| 2 | Helpers format (formatXOF, formatDate, formatQuantity) | ✅ |
| 3 | API produits (GET list, POST create) | ✅ |
| 4 | API produits detail, update, delete, config entrepôt | ✅ |
| 5 | Stock service avec calcul PUMP (transactions atomiques) + tests | ✅ |
| 6 | Validators mouvements + API mouvements CRUD + tests | ✅ |
| 7 | Composant scanner codes-barres (html5-qrcode) | ✅ |
| 8 | Page liste produits avec recherche et filtres | ✅ |
| 9 | Page création produit avec validation formulaire | ✅ |
| 10 | Page liste mouvements avec filtres | ✅ |
| 11 | Page édition produit | ✅ |
| 12 | Page détail produit (stock par entrepôt + mouvements) | ✅ |
| 13 | Page création mouvement avec validation stock | ✅ |
| 14-16 | Corrections code review (2 passes, 4 critiques résolues) | ✅ |

**Tests :** 56 tests (services stock, validators, mouvements), 35 fichiers modifiés

### 12.5 Semaine 3 — Transferts, Inventaire & Résilience (16-17 fév) ✅

**Branche :** `feat/week3-transfers-inventory-resilience` → merged via PR #3 (14 tâches)

| # | Tâche | Statut |
|---|-------|--------|
| 1 | Service d'audit (log + getByEntity) + tests | ✅ |
| 2 | Service d'alertes (stock alerts, déduplication, read tracking) + tests | ✅ |
| 3 | Validators transferts (Zod + refinements) | ✅ |
| 4 | Service transferts (machine à 8 états) + tests | ✅ |
| 5 | API transferts (CRUD + actions workflow) | ✅ |
| 6 | Pages transferts (liste, détail, création) | ✅ |
| 7 | Validators inventaire (Zod) | ✅ |
| 8 | Service inventaire (sessions, comptage, validation) + tests | ✅ |
| 9 | API inventaire (CRUD sessions) | ✅ |
| 10 | Pages inventaire (liste, création, comptage) | ✅ |
| 11 | Intégration alertes (triggers stock + transferts) | ✅ |
| 12 | Résilience réseau (offline queue IndexedDB, network store, banner) | ✅ |
| 13 | Tests d'intégration + isolation | ✅ |
| 14 | Corrections code review | ✅ |

### 12.6 Seed Data (17 fév) ✅

**Branche :** `feat/seed-demo-data` → merged via PR #4

| Donnée | Quantité |
|--------|----------|
| Utilisateurs | 6 (admin, admin_manager, manager, user, admin_viewer, viewer) |
| Entrepôts | 4 (Dakar Centre, Dakar Port, Thiès, Saint-Louis) |
| Catégories | 8 (5 parents + 3 sous-catégories) |
| Produits | 15 (matériaux construction) |
| Stock entries | 35 (répartis sur 4 entrepôts) |
| Transferts | 4 (pending, approved, shipped, received) |
| Inventaires | 1 session (6/8 articles comptés) |
| Alertes | 5 (stock bas, transferts) |
| Audit logs | 8 |

### 12.7 Semaine 4 — Dashboard, Polish & Déploiement (à venir)

| # | Tâche | Statut | Priorité |
|---|-------|--------|----------|
| 1 | API dashboard (KPIs par rôle + scope) | ⏳ | Haute |
| 2 | Page dashboard (KPIs, graphiques mouvements, stock bas, actions rapides) | ⏳ | Haute |
| 3 | Service audit complet + page logs (filtres, pagination, export CSV) | ⏳ | Haute |
| 4 | Page logs (avant/après diff, métadonnées) | ⏳ | Moyenne |
| 5 | Polish mobile (formulaires tactiles, cards responsive, pull-to-refresh) | ⏳ | Moyenne |
| 6 | Loading states (skeleton screens) | ⏳ | Moyenne |
| 7 | Empty states (illustrations + CTA) | ⏳ | Basse |
| 8 | Confirmation modals (actions destructives) | ⏳ | Moyenne |
| 9 | Migrations production (db:generate + review SQL) | ⏳ | Haute |
| 10 | Tests E2E Playwright (auth, mouvements, transferts) | ⏳ | Haute |
| 11 | Build production + déploiement Cloudflare | ⏳ | Haute |
| 12 | Smoke tests production | ⏳ | Haute |

### 12.8 Pull Requests

| PR | Titre | Branche | Statut | Date merge |
|----|-------|---------|--------|------------|
| #1 | Week 1 — Foundations | `feat/week1-foundations` | ✅ Merged | 11 fév |
| #2 | Week 2 — Core Business | `feat/week2-core-business` | ✅ Merged | 16 fév |
| #3 | Week 3 — Transfers, Inventory & Resilience | `feat/week3-transfers-inventory-resilience` | ✅ Merged | 16 fév |
| #4 | Seed demo data | `feat/seed-demo-data` | ✅ Merged | 17 fév |

### 12.9 Métriques

| Métrique | Valeur |
|----------|--------|
| Commits total (main) | ~60 |
| PRs merged | 4 |
| Tables DB | 16 |
| Tests unitaires | 56+ |
| Fichiers source | 100+ |
| Pages UI fonctionnelles | 15+ |
| Avancement global | **~75%** (3/4 semaines) |

---

**Document rédigé le :** 5 février 2026
**Dernière mise à jour :** 17 février 2026
**Prochaine mise à jour :** Après début Semaine 4
